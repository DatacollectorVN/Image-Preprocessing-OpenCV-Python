...
import os
import argparse
from src.edge_detector import laplacian_function

def main(img_name, output_name, img_dir='./imgs', lap_ksize=3):
    IMAGE_PATH = os.path.join(img_dir, img_name)
    img = cv2.imread(IMAGE_PATH)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) 
    lap_img = laplacian_function(img, lap_ksize = lap_ksize)
