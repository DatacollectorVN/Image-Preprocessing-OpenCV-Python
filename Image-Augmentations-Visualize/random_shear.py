...
from src.data_augmentation import RandomShear
import argparse
...
def main(img_file=img_file, annotations_file=annotations_file, color=color, output_name=None):
        ...
        random_shear = RandomShear(shear_factor = (0.1, 0.2), p = 1)
        shear_img, shear_bboxes, shear_labels = random_shear(img, bboxes, labels)
        ...
        # draw scale img
        for i in range(len(shear_bboxes)):
            draw_bboxes(shear_img, bbox = shear_bboxes[i], label = shear_labels[i], color = color[shear_labels[i]], thickness = 3)

        cv2.imshow('original-img', img)
        cv2.imshow('flip-img', shear_img)
        cv2.waitKey(0)

        if output_name is not None:
            os.makedirs('./outputs', exist_ok = True)
            #scv2.imwrite(os.path.join('./outputs', 'original-img.jpg'), img)
            cv2.imwrite(os.path.join('./outputs', output_name), shear_img)
            print('Done saved')
