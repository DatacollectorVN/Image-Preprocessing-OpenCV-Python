...
from src.data_augmentation import RandomRotate
import argparse
...
def main(img_file=img_file, annotations_file=annotations_file, color=color, output_name=None):
        ...
        random_rotate = RandomRotate(angle = 90, p = 1)
        rotate_img, rotate_bboxes, rotate_labels = random_rotate(img, bboxes, labels)
        ...
        # draw scale img
        for i in range(len(rotate_bboxes)):
            draw_bboxes(rotate_img, bbox = rotate_bboxes[i], label = rotate_labels[i], color = color[rotate_labels[i]], thickness = 3)

        cv2.imshow('original-img', img)
        cv2.imshow('flip-img', rotate_img)
        cv2.waitKey(0)

        if output_name is not None:
            os.makedirs('./outputs', exist_ok = True)
            #scv2.imwrite(os.path.join('./outputs', 'original-img.jpg'), img)
            cv2.imwrite(os.path.join('./outputs', output_name), rotate_img)
            print('Done saved')
