...
from src.data_augmentation import Resize
import argparse
...
def main(img_file=img_file, annotations_file=annotations_file, color=color, output_name=None):
        ...
        resize = Resize(inp_dim = (256, 256))
        resize_img, resize_bboxes, resize_labels = resize(img, bboxes, labels)
        ...
        # draw scale img
        for i in range(len(resize_bboxes)):
            draw_bboxes(resize_img, bbox = resize_bboxes[i], label = resize_labels[i], color = color[resize_labels[i]], thickness = 1)

        cv2.imshow('original-img', img)
        cv2.imshow('flip-img', resize_img)
        cv2.waitKey(0)

        if output_name is not None:
            os.makedirs('./outputs', exist_ok = True)
            #scv2.imwrite(os.path.join('./outputs', 'original-img.jpg'), img)
            cv2.imwrite(os.path.join('./outputs', output_name), resize_img)
            print('Done saved')
